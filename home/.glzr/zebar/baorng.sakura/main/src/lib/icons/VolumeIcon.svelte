<script lang="ts">
  ...
  let path = $derived.by(() => {
    ...
    if (volumePercentage < 25) return LOW;
    if (volumePercentage < 50) return MEDIUM;
    return HIGH;
  });
</script>
